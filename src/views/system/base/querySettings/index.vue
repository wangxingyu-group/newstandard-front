      ...
      <el-col :span="24">
        <el-card>
          ...
          <el-table ref="table" :key="0" v-loading="tableLoading" :data="tableData" :height="searchRow1" row-key="id" stripe highlight-current-row @selection-change="selectionChange">
            ...
            <el-table-column label="查询字段" align="center" min-width="100">
              <template slot-scope="scope">
                <span>{{ scope.row.queryField }}</span>
              </template>
            </el-table-column>
            <el-table-column label="查询名称" align="center" min-width="100">
              <template slot-scope="scope">
                <span>{{ scope.row.quertName }}</span>
              </template>
            </el-table-column>
            <el-table-column label="查询类型" align="center" min-width="100">
              <template slot-scope="scope">
                <span>{{ scope.row.queryType }}</span>
              </template>
            </el-table-column>
            <el-table-column label="所属功能" align="center" min-width="100">
              <template slot-scope="scope">
                <span>{{ scope.row.function }}</span>
              </template>
            </el-table-column>
            <el-table-column label="禁用" align="center" min-width="100">
              <template slot-scope="scope">
                <span>{{ scope.row.disabled }}</span>
              </template>
            </el-table-column>
            <el-table-column label="只读" align="center" min-width="100">
              <template slot-scope="scope">
                <span>{{ scope.row.readonly }}</span>
              </template>
            </el-table-column>
