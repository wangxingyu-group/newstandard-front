      ...
      <table cellpadding="5">
        ...
        <tr v-for="(day,dayIndex) in days">
          <td v-for="(child,childIndex) in day" :class="{'selected':child.selected,'disabled':child.disabled}" @click="select(dayIndex,childIndex,$event)" @touchstart="select(k1,k2,$event)">
            <span>{{ child.day }}</span>
            <div v-if="showLunar" class="lunar">{{ child.lunar }}</div>
          </td>
        </tr>
      </table>
    </div>
    <div v-show="type=='datetime'||type=='time'" class="calendar-time">
      <div class="timer">
        <input v-model="hour" type="text" :value="hour" min="0" max="23" maxlength="2"> 时
        <input v-model="minute" type="text" :value="minute" min="0" max="59" maxlength="2"> 分
        <input v-model="second" type="text" :value="second" min="0" max="59" maxlength="2"> 秒
      </div>
    </div>
    <div v-show="type=='datetime'||type=='time'||range" class="calendar-button">
